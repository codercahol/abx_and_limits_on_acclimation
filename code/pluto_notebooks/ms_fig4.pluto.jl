...
# ╔═╡ 671897aa-3ab1-4ddb-9fe9-8b1fc2e63ce4
md"""
### 4A
"""

# ╔═╡ 76515f5d-abe7-4c68-8df3-b86351376505
md"""
## 4B
"""

# ╔═╡ d3a35812-0c87-4ca5-aec6-67e5de3774f5
begin
	abx_conc =  f.Cm_ug_per_mL_to_M(15)
	period = 3
	duty = 0.2
	duration = period*10
	xxs_3 = range(0,0.015, length=30)
	yys_3 = [f.GR_given_amin_multi.(
		xxs_3, 4, abx_conc*0.25, period,duty,duration,n_responsive=i
	) for i in 0:3]
end

# ╔═╡ 288933d8-15db-4cea-92c5-de4c2450865d
begin
	n_abx = 2
	pm_multi_base = f.default_PcMultiABXwAnt_params(n_abx)
	#  u0 set by ss in no ABX 
	u0_multi = f.full_state_to_u0_multiABX(f.dSS_solver(pm_multi_base, n=n_abx))

	#period = 3
	#duty = 0.2
	#duration = period*10
	tspan_cyc = (0.0, duration)

	abx_conc2 = 10
	C_max = f.Cm_ug_per_mL_to_M(abx_conc2)
	
	Random.seed!(1) # 1 for n=2, 6 for n=3
	random_start = vcat(0, rand(n_abx-1)) .* period
	#non_random_start = vcat(0, zeros(n_abx-1)) .* period
	abx_flucts(i, t) = [
		f.square_wave(period, duty, duration, amplitude=C_max, x0 = random_start[i]) for i in 1:n_abx
	][i](t)
	pm_multi_ABX = f.mutate(pm_multi_base, C_max = abx_flucts)

	fluct_params_vec = [(period, duty, duration, x) for x in random_start]
end

# ╔═╡ 3401eec3-059e-4594-ab0d-8efebaa42908
begin
	precis = 1e-7
	prob = ODEProblem(f.growth_dynamics, u0_multi, tspan_cyc, pm_multi_ABX)
	sol = solve(prob, Rodas4(), abstol = precis, reltol = precis)
end

# ╔═╡ ce437483-eb81-43cb-9ca9-27f1afa5fd50
begin
	all_p = f.plot_DE_soln(sol, pm_multi_ABX, fluct_params_vec=fluct_params_vec, v_lines=true)
	mABX_p = plot(all_p...; layout = grid(2,2), size=(900, 700), legend=:topright)
end

# ╔═╡ b11793ae-d3ff-4dc2-8dd1-e5e8e081325f
plot(mABX_p[1], xlims=(0,10))

# ╔═╡ 587cfe3d-ad24-4587-9c32-067b25aee827
plot(mABX_p[2], xlims=(0,10),ylims=(0,2))

# ╔═╡ 553222cb-814f-4a5b-b5ae-e1622da3228a
begin
	xxs_1 = range(0,0.06, length=101)
	yys_1 = [f.GR_given_amin.(
		xxs_1, abx_conc*i*4*0.25, period,duty,
	) for i in 1:4]
end

# ╔═╡ 06e037ae-19f1-4f18-810e-a4216eb96ade
begin
	cs3 = cgrad(:navia, 5, categorical=true)
	pp_13=plot(xlabel="Anticipatory Mass Fraction", ylabel="Growth Rate \n(Relative to Growth with No Resistance)", legend=:bottomright, ylims=(0.98,1.15))
	hline!([1], label="0% Anticipation", lw=3, xlims=(0,0.045), color=colorant"bisque", dpi=500)
	for i in [4,3,2,1,0]
		j = 4 - i
		if j >0
			xxs2 = xxs_3 * j
		...
		plot!(xxs2, yys_3[i+1]./yys_3[i+1][1],label="$((1 - i/4) * 100)% Anticipation", lw=3, color=cs3[i+1])
	end
	plot!(xxs_1, yys_1[1]./yys_1[1][1],label="Single Antibiotic", lw=2, color=colorant"black", ls=:dashdot)
	pp_13
end

# ╔═╡ 44027d68-f429-4c12-8b5f-daa90e7341ab
md"""
## 4C
"""

# ╔═╡ 94b11f63-13c9-4739-8794-8a5004839c23
begin
	qqq = 6 # n_abx
	rezies = zeros(qqq,qqq+1) .* NaN
	total_ant = zeros(qqq,qqq+1) .* NaN
	for ii in 1:qqq
		# only 1 ABX/phi_AR
		if ii == 1
			# single ant strat
			ant_strat_out = f.opt_amin(abx_conc*0.5, period, duty)
			rezies[1,1] = ant_strat_out[2]
			total_ant[1,1] = ant_strat_out[1][1]
			# single responsive strat
			total_ant[1,2]=0
			rezies[1,2]= f.GR_given_amin(0.0, abx_conc,period, duty)			
		else
			for kk in 0:ii
				# normalized so total abx is const
				res = f.opt_amin_multi(ii, abx_conc*0.5/ii, period, duty, duration, n_responsive=kk)
				rezies[ii, kk+1] = res[2]
				total_ant[ii, kk+1] = res[1][1]*(ii-kk)
			end
		end
	end
end

# ╔═╡ f6d844b0-baa0-4413-a743-d95cc3607428
p_multi_GR = heatmap(numba_abx, numba_ant_strat, rezies', xlabel="Number of Antibiotics", ylabel="Number of Responsive Strategies", cbar_title=" \n"*"Growth Rate (1/hr)", right_margin=5mm, title="", colorbar=true, size=(650,500), dpi=500, color=cs2)

# ╔═╡ 77f84cd7-af2f-405c-a09c-ba77d6e7b96b
p_multi_phiAR = heatmap(numba_abx, numba_ant_strat, total_ant', xlabel="Number of Antibiotics", ylabel="Number of Responsive Strategies", cbar_title=" \n"*"Anticipatory Mass Fraction", right_margin=5mm, title="", colorbar=true, size=(650,500), dpi=500, color=cs)
