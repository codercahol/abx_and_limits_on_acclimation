...
# ╔═╡ 6b1c2170-6436-11f0-07a7-8b4826798c23
begin
...
# ╔═╡ 097cca67-12e1-4314-a1a4-a2913f807b18
...
# ╔═╡ f70842e6-b168-4321-897f-296375b2db2d
md"""
...
# ╔═╡ f589df1c-b381-4549-a0d2-9580227e5bf0
begin
	using JSON
	json_dict = JSON.parsefile("../secrets/paths_to_repos.json")
end

# ╔═╡ 4be5331d-68dc-42c7-85d2-37439e2e1e38
begin
...
# ╔═╡ 656eea06-094b-44cc-805c-68854d9946f2
md"""
# Figures 3 and S5
"""

# ╔═╡ 1819e57c-f7eb-4d91-b521-fc593614ba8d
md"""
## Fig 3A
"""

# ╔═╡ 6c6e3902-9e80-4326-8333-43e7013c3013
begin
	abx_conc = 15 # ug/mL Cm
	zs = range(-10, 50, length=5001)
	plot(zs, f.square_wave(3, 0.2, 40, x0=0, amplitude=abx_conc)(zs), label="", lw=3, c=:black, xlims=(0,8), ylabel="Antibiotic Concentration (μg/mL)", xlabel="Time (hrs)", size=(800,400), leftmargin=0mm, bottommargin=0mm)
	plot!(minorgrid=true, gridalpha=0.2, minorgridalpha=0.1, tickfontsize=10, labelfontsize=12, legendfontsize=9, title="", dpi=500, size=(500,400))
end

# ╔═╡ 83398780-d2e7-495d-b159-3a28ead3eee3
md"""
## Fig 3B
"""

# ╔═╡ 94a046d2-ffd6-452c-a3f3-e6d485a19f35
begin
	# takes ~5200 seconds to run
	fname = "opt_amin_PD.jld2"
	opt_amins = f.cache_function_output(fname, f.compute_opt_amin_matrix, overwrite=false, duties=0:0.01:1, periods=1:1:100, abx_conc=f.Cm_ug_per_mL_to_M(15))
end

# ╔═╡ d1ea87bc-d623-4fab-94a5-996a50e90503
begin
	cs =  cgrad(:navia, rev=true)
	duties=0:0.01:1
	periods=1:1:100
end

# ╔═╡ 59b7e473-3631-4aae-b150-10a10d3e18ce
p_o_amins = heatmap(periods, duties, opt_amins, xlabel="Period (hrs)", ylabel="Duty", cbar_title=" \n"*"Anticipatory Mass Fraction", right_margin=5mm, title="", colorbar=true, size=(650,500), dpi=500, clims=(0,0.01), color=cs)

# ╔═╡ 68bb5c75-f50d-414d-9a3e-2b3492b3c65b
md"""
## Fig 3C
"""

# ╔═╡ c83068d9-359c-4c36-939a-365eda87f32a
begin
	amis2 = range(0, 0.04, length=201)
	amis = range(0, 0.04, length=51)
	p3 = 5
	d3 = 0.5
	abx_swp = f.Cm_ug_per_mL_to_M(15)
	abx2 = f.Cm_ug_per_mL_to_M(100)
	y4s(ae) = f.GR_given_amin(ae, abx2, p3, d3, precision=1e-7, solver=Rodas4())
	y3s(ae) = f.GR_given_amin(ae, abx_swp, p3, d3, precision=1e-7, solver=Rodas4())
	...
	# rescale 
	y4 = y44 ./ maximum(y44)
	ye = yees ./ maximum(yees)
	
	p = hline([1.0], ls=:dash, color=:black, label="", xlims=(0,maximum(amis2)))
	plot!(amis2, y4, label=L"100 \mu g"*"/mL Cm", xlabel="Anticipatory Mass Fraction", ylabel="Normalized Growth Rate", color=colorant"darkslategray3", lw=3)
	plot!(amis, ye, ylims=(0.9,1.01), label=L"15 \mu g"*"/mL Cm", color=:black, lw=3, dpi=500)
end

# ╔═╡ 3f7c7506-d4ed-4f67-bf5d-df3c4957f735
p

# ╔═╡ 3fa5257a-52be-486d-a374-6d2aba6d6756
md"""
## Fig 3D and S5
"""

# ╔═╡ c7715b12-1b83-436a-a492-b832398c5cd4
begin
	color_scheme = cgrad(:roma, rev=true)
	cs2 = :delta
	amin = 0.005
	duties_2 = 0:0.01:1
end

# ╔═╡ aaf1c851-f1bd-4602-a637-5a8c3a25dd0a
begin
	file_PD_ant_long = "PD_ant_long_periods.jld2"
	periods_long = 1:1:100
	grs_ant_long = f.cache_function_output(file_PD_ant_long, f.growth_rate_phase_diagram_mtx, duties_2, periods_long, AR_strategy="anticipatory", overwrite=false, normalize_by_max_gr=false, C_max=f.Cm_ug_per_mL_to_M(abx_conc), ant_αmin=amin)
end

# ╔═╡ 6246c4fd-ac15-42e1-9009-c926977ab36b
begin
	file_PD_resp_long = "PD_resp_long_periods.jld2"
	grs_resp_long = f.cache_function_output(file_PD_resp_long, f.growth_rate_phase_diagram_mtx, duties_2, periods_long, AR_strategy="responsive", overwrite=false, normalize_by_max_gr=false, C_max=f.Cm_ug_per_mL_to_M(abx_conc), ant_αmin=amin) #ow - true
end

# ╔═╡ 0217039f-a5ca-4087-8af0-c2a023e75a9f
begin
	grs_comp_long = grs_ant_long .- grs_resp_long
	pcent_gr_change = grs_comp_long./grs_resp_long.*100
	mask1 = abs.(pcent_gr_change) .>= 1
	mask2 = abs.(grs_comp_long) .>= .005
	percent_gr_change_thresh1 = mask1 .* pcent_gr_change
	thresh2 = mask2 .* grs_comp_long
	mod2_data = copy(thresh2)
	mod2_zeros = copy(thresh2)
	mod2_data[thresh2 .== 0.0] .= NaN
	mod2_zeros[thresh2 .!= 0.0] .= NaN
end

# ╔═╡ 548dc00c-1091-4bce-972b-6b097c48bd7a
begin
	hm2 = heatmap(periods_long, duties_2, mod2_zeros, c=:gray, colorbar=false)
	d_comp_pcent_thresh2 = heatmap!(hm2, periods_long, duties_2, mod2_data, color=cs2, clim = (-0.1, 0.1), xlabel="Period (hrs)", ylabel="Duty", cbar_title=" \n\n"*L"$\Delta$ Growth Rate (1/hr)", right_margin=10mm, title="", colorbar=true, size=(650,500))
	plot!(tickfontsize=10, labelfontsize=12, legendfontsize=9, title="", dpi=500)
end

# ╔═╡ 856c570d-0331-490f-aa40-78ac98317d70
# unthreshholded image
pd_comp_long = heatmap(periods_long, duties_2, grs_comp_long, color=cs2, clim = (-0.1, 0.101), xlabel="Period of Fluctuations (hrs)", ylabel="Duty", cbar_title="\n\n"*L"$\Delta$Growth Rate (1/hr)", right_margin=8mm, title="",xlims=(0,100))

# ╔═╡ 7fbedce1-5d3c-4ae0-b4ea-751bd18cffa2
md"""
### Fig S5
"""

# ╔═╡ 00e31205-4ea3-41eb-b061-98486b346829
#anticipatory
pd_ant_long = heatmap(periods_long, duties_2, grs_ant_long, color=color_scheme, xlabel="Period (hrs)", ylabel="Duty", cbar_title="", title="Anticipation", clim=(0.85, 1), size=(550,400), rightmargin=3mm)

# ╔═╡ 95b70432-e523-4ad7-ba29-1a4d630ea306
#responsive
pd_resp_long = heatmap(periods_long, duties_2, grs_resp_long, color=color_scheme, xlabel="Period (hrs)", ylabel="Duty", cbar_title="", colorbar=false, title="Responsive", size=(500,400), clim=(0.85,1))
...
manifest_format = "2.0"
project_hash = "d99ec0ffd519a7b41dbac02f3292e87dd5495cb3"
