...
# ╔═╡ cb307298-6371-11f0-150b-6128d03eea1c
begin
...
# ╔═╡ a8b75f3c-3cf2-4e50-ade3-5a75247d6b6a
begin
	using JSON
	json_dict = JSON.parsefile("../secrets/paths_to_repos.json")
end

# ╔═╡ 55dba04c-0002-4cc8-b70e-f19202d8a39d
...
# ╔═╡ c15c993c-5839-4f55-b429-361484a12c81
md"""
...
# ╔═╡ 76a7583a-1be0-4322-9979-27f0b8afb82b
begin
	path_to_AbxGrowthSims = json_dict["path_to_AbxGrowthSims"]
	f = ingredients(path_to_AbxGrowthSims * "/src/ABXGrowthSims.jl").AbxGrowthSims
end

# ╔═╡ 9217fd99-1e66-4bdd-ad99-b861a42c41ed
md"""
# Code Supplement for Fig 1, S1, and S2
"""

# ╔═╡ dde63f17-826a-4598-8920-a6231730fc9a
md"""
# Fig 1
...
# ╔═╡ 29ae08a1-3d67-412a-bd10-5b0f644a787c
begin
	rRNA_per_totalRNA = 0.85
	RtoP = 1.0 / 1.7 # MDa protein / MDa RNA in a ribosome
	
	RNA_cal = rRNA_per_totalRNA * RtoP #* RNA_AA_mass_ratio
	
	# Dai et al 2016 data
	# chloramphenicol concentrations
	Cm_gluc0p2 = [0,2,4,6,8,9] .* 1e-6 # M
	GR_gluc0p2 = [0.98, 0.71,0.53, 0.41, 0.33, 0.26] # 1/h
	pR_gluc0p2 = [0.294, 0.358, 0.440, 0.487, 0.511, 0.569] .* RNA_cal

	Cm_rich = [0,2,4,6,8].* 1e-6
	GR_rich = [1.8, 1.46, 1.08, 0.87, 0.57]
	pR_rich = [0.476, 0.495, 0.551, 0.582, 0.621] .* RNA_cal

	Cm_fruct = Cm_rich
	GR_fruct = [0.69, 0.46, 0.35, 0.27, 0.21]
	pR_fruct = [0.217, 0.286, 0.323, 0.394, 0.457] .* RNA_cal

	Cm_acet = [0,3,6] .* 1e-6
	GR_acet = [0.46, 0.25, 0.18]
	pR_acet = [0.172, 0.246, 0.304] .* RNA_cal

	Cm_aspart = [0,2,4] .* 1e-6
	GR_aspart = [0.33, 0.24, 0.17]
	pR_aspart = [0.152, 0.231, 0.292] .* RNA_cal

	NCM_gluc_MM_phiR = 0.13
	conv_to_uM = 3.095 # ug/mL to uM
	gluc_MM_NCM_gr_LPz = [1.08, 0.894897689583001, 0.7526142828963739, 0.5131844952880615, 0.4368807194065612, 0.2866414307176571, 0.22815118940595613, 0.19756834798013434, 0.18330878959552818]
	ugml = [0, 0.75, 1.5, 2.25, 3, 3.75, 4.5, 5.25, 6]

	Dai_vs_my_data = Dict(
		"Glucose 0.2%"=>(Cm_gluc0p2, GR_gluc0p2, pR_gluc0p2),
		"Rich Media"=>(Cm_rich, GR_rich, pR_rich),
		"Fructose"=>(Cm_fruct, GR_fruct, pR_fruct),
		"Acetate"=>(Cm_acet, GR_acet, pR_acet),
		"Aspartate"=>(Cm_aspart, GR_aspart, pR_aspart),
		"This study"=>(ugml * conv_to_uM * 1e-6, gluc_MM_NCM_gr_LPz, [NCM_gluc_MM_phiR])
	)
end

# ╔═╡ cd3920da-4da8-426b-9b0c-af5924feaea6
begin
	file_ABX_vSS = "fig1_B_data.jld2"
	params = f.default_PcABXwAnt_params()
	p123, data123 = f.cache_plot_data(file_ABX_vSS, f.plot_GR_vs_ABX_Dai_curves, Dai_vs_my_data, params; overwrite=true, opt=f.dSS_solver)
	p123
end

# ╔═╡ 32b8c69f-cf24-42cf-86dc-03317f9b73f1
begin
	# data from Chure & Cremer
	using CSV
	path = "../data/ChureCremer_ribosomal_growth_data.csv"
	df = CSV.read(path, DataFrame; header=9, skipto=10)
	gr_df = df.growth_rate_hr
	phiR_df = df.mass_fraction
	
	# empirical relation
	nus = collect(range(0.1, 21, length = 501))
	nu_dict = Dict(:νmax=>nus)
	base_pm = f.mutate(params, C_max=0, αmax = 0.0, αmin = 0.0)
	nu_sweep = f.param_sweep_ss(nu_dict, base_pm, opt=f.SS_optimizer_v4, opt_kwargs=Dict(:V=>2, :K=>0.8, :n=>1))
	cpcs2 = []
	Rs2 = []
	Ls2 = []
	for pt in nu_sweep
	...
	pms = [f.mutate(base_pm, νmax = nu) for nu in nus]
	analytic_opt_Ls = f.analytic_optimal_growth.(pms)
	analytic_opt_Rs = f.optimal_φ_R.(pms)
end

# ╔═╡ 227a3272-ce62-4958-a922-cae270c1d598
md"""
## Fig 1C & S2
"""

# ╔═╡ 36f8f430-7bdb-487b-b243-5938acf026ce
begin
	col_AR = colorant"#E0BF00"
	col_abx = colorant"#F26165"
end

# ╔═╡ ef9a5c69-961e-4744-8966-1b2deca0713a
begin
	# Deris data (normalized by λ_0)
	cat3_GR = [1, 0.918, 0.711, 0.567, 0.497, 0.065, 0.009]
	cat3_ABX = [0, 100,200,300,350,400,500] .* 1e-6 # uM external
	cat3 = (cat3_GR, cat3_ABX)

	cat2_GR = [1, 0.840, 0.808, 0.667, 0.615, 0.026, 0.0, 0.00]
	cat2_ABX = [0, 200, 300, 400, 500, 600, 700, 800] .* 1e-6
	cat2 = (cat2_GR, cat2_ABX)

	cat1_GR = [1, 0.904, 0.895, 0.861, 0.796, 0.693, 0.600, 0.514, 0.0, 0.0]
	cat1_ABX = [0, 2, 3, 4, 5, 7, 8, 9, 10, 11] .* 100 .* 1e-6
	cat1 = (cat1_GR, cat1_ABX)
	
	WT_GR = [1, 0.848, 0.651, 0.699, 0.512, 0.517, 0.452, 0.402, 0.36, 0.354, 0.275, 0.230]
	WT_ABX = [0, 1,2,2.5, 5,2,6,8,8,10, 10,13] .* 1e-6
	catW = (WT_GR, WT_ABX)

	cat6_GR = [1, 0.838, 0.677, 0.505, 0.426, 0.385, 0.340, 0.22, 0.23, 0.156]
	cat6_ABX = [0, 10, 15, 20, 25, 30, 35, 30, 40, 50] .* 1e-6
	cat6 = (cat6_GR, cat6_ABX)

	cat5_GR = [1, 0.857, 0.625, 0.515, 0.404, 0.165, 0.05, 0.0]
	cat5_ABX = [0, 25, 50, 75, 100, 125, 150, 170] .* 1e-6
	cat5 = (cat5_GR, cat5_ABX)

	abs_gr_CAT1 = [0.583, 0.468, 0.316]
	abs_ABX = [0, 7e-4, 9e-4]

	Deris_set = [cat1, cat2, cat3, cat5, cat6, catW]

	# in MG1655, form Scott et al 2010
	pR_gluc = [0.23] .* RNA_cal
	normalizing_GR = abs_gr_CAT1[1]
	λ_0 = [normalizing_GR]

	kcat_full = 600*60^2
	# fit by hand from Deris figures
	kcats = kcat_full .* [1, 0.66, 0.4, 0.075, 0.025, 0]

	fAR = 0.01
	pm_1 = f.mutate(base_pm, ke=kcat_full, αmax = fAR, αmin = fAR)
	# log range
	abx_rang2 =  collect(10. .^ range(-7,log10(0.0012), length=101))
	abx_rang2 = vcat(0, abx_rang2)
end

# ╔═╡ b811f70e-eb0f-461b-b3f3-458932f60239
md"""
### Precursor Responsive Allocation (Fig 1C and S2)
"""

# ╔═╡ b44baeba-1016-45e6-aa81-e76f98c3e363
md"""
### Optimal Proteome Allocation (Fig S2)
"""

# ╔═╡ b825b062-8117-42d3-bb3d-196c56c565b3
begin
	opt1_staticii = []
	allopt1states = []
	abx_range_no0 =  collect(10. .^ range(-7,log10(0.0012), length=101))
	abx_range_no0 = vcat(eps(), abx_range_no0)
	for i in 1:6
		pm_ = f.mutate(pm_1, ke=kcats[i])
		out = f.sweep_ss(f.SingleSolver(), pR_gluc[1], λ_0[1], abx_range_no0, pm_, opt=f.abxOpt_solver)
		normed_gr = out[:,2] ./ normalizing_GR
		push!(opt1_staticii, normed_gr)
		push!(allopt1states, out)
	end
end

# ╔═╡ 299a7e73-b199-428f-a1d3-8a11e3f68fe1
md"""
### Optimal Proteome Allocation (Fig S2)
"""

# ╔═╡ 203706f4-d662-411d-bf43-d0c39f7faead
begin
	true_staticii = []
	all_true_states = []
	for i in 1:6
		pm_ = f.mutate(pm_1, ke=kcats[i])
		out = f.sweep_ss(f.SingleSolver(), pR_gluc[1], λ_0[1], abx_rang2, pm_, opt=f.allOpt_solver)
		normed_gr = out[:,2] ./ normalizing_GR
		push!(true_staticii, normed_gr)
		push!(all_true_states, out)
	end
end

# ╔═╡ 9576d7d5-8061-43d7-894e-da9c2c50e8ec
begin
	# optimal proteome plot in paper
	c_range_ms = f.gen_color_range(col_abx, col_AR)
	colors_ms = c_range_ms[round.(Int, range(1,100,length=6))]

	tru_p_ms = scatter(catW[2], catW[1]*normalizing_GR, label="", xlabel = "Antibiotics [M]", ylabel = "Growth Rate (hr^-1)", color=colors_ms[6], title="Optimal Proteome")
	plot!(abx_rang2, true_staticii[6]*normalizing_GR, label="",color=colors_ms[6], size=(500,400), lw=2.5)
	for i in 1:5
		scatter!(Deris_set[i][2], Deris_set[i][1]*normalizing_GR, label="", color=colors_ms[i])
		plot!(abx_rang2, true_staticii[i]*normalizing_GR, label="",color=colors_ms[i], lw=2.5)
	end
	tru_p_ms
end

# ╔═╡ c539a241-ca0a-451c-b2df-762cf0862b39
begin
	# precursor responsive plot in paper
	poipoi_ms = scatter(catW[2], catW[1]* normalizing_GR, color=colors_ms[6])
	Deris_nu = f.find_nu(pR_gluc[1], λ_0[1], pm_1)
	pm_BK = f.mutate(pm_1, kp=760, νmax = Deris_nu)
	for i in 1:6
		pm_ = f.mutate(pm_BK, ke=kcats[i])
		br, probb = f.bifurcation_SS_solver(pm_)
		plot!(br, color=colors_ms[i],label="")
		scatter!(Deris_set[i][2], Deris_set[i][1] * normalizing_GR, label="", color=colors_ms[i])
	end
	plot!(xlabel = "Antibiotics [M]", ylabel = "Growth Rate (hr^-1)", legend=:none, title="", size=(500,400), xlims=(-3e-5,0.0012), dpi=500)
	poipoi_ms
end

# ╔═╡ d11500e5-d643-4614-9d67-de03b6a503af
begin
	# breaks for the continuous case (no breakpoints)
	j4 = 2
	ms_pm_bk_i = f.mutate(pm_BK, ke=kcats[j4])
	ms_br_heur, _ = f.bifurcation_SS_solver(ms_pm_bk_i)
	ms_full_bk_states, ms_abx_bk = f.bifurc_soln_to_full_state(ms_br_heur, ms_pm_bk_i)

	# don't plot (overlapping) bifurcation
	val_idxs = ms_abx_bk[2] .>= ms_abx_bk[1][end] - 0.000001
		
	# to exaggerate the size of AR
	factor = 10
	xx = ones(size(ms_full_bk_states[1]))
	xx[:,2] *= factor
	x3 = ones(size(ms_full_bk_states[2][val_idxs,:]))
	x3[:,2] *= factor

	prot_colors = [f.φR_color f.φAR_color f.φM_color]
	labs = reshape([L"\phi_R",L"\phi_{AR}",L"\phi_M"], (1,3))
	# heuristic model, branch 1
	h_1 = areaplot(ms_abx_bk[1], ms_full_bk_states[1] .* xx, legend=:outerright, labels=labs, fillalpha = [0.7 .7 .7],
		seriescolor = [f.φR_color f.φAR_color f.φM_color], linealpha = 0, title="Precursor Responsive Allocation Cartoon", ylabel=L"1-\phi_0",xlabel="ABX [M]", ylims=[0, 1 - pm_1.abx.φ_0])

	# don't plot bifurcation
	h_both_ms = areaplot!(h_1, ms_abx_bk[2][val_idxs], ms_full_bk_states[2][val_idxs,:] .* x3, legend=false, labels="", fillalpha = [.7 .7 .7],#[0.3 1 0.3],
		seriescolor = prot_colors, linealpha = 0, ylabel="",xlabel="Antibiotics [M]", yticks=false, size=(500,400))
end

# ╔═╡ 3e7efc1c-3db0-4b0f-bd10-8a1d3ed5028f
begin
	# optimal ribosome plot in paper
	opti_p_ms = scatter(catW[2], catW[1]*normalizing_GR, label="", xlabel = "Antibiotics [M]", ylabel = "Growth Rate (hr^-1)", color=colors_ms[6], title="", size=(500,400), dpi=600)
	plot!(abx_rang2, opt1_staticii[6]*normalizing_GR, label="",color=colors_ms[6], lw=2.5)
	for i in 1:5
		scatter!(Deris_set[i][2], Deris_set[i][1]*normalizing_GR, label="", color=colors_ms[i])
		plot!(abx_rang2, opt1_staticii[i]*normalizing_GR, label="",color=colors_ms[i], lw=2.5)
	end
	opti_p_ms
end

# ╔═╡ 4230ed00-7288-4321-983e-754cba042390
begin
	prot_colors2 = [f.φAR_color f.φM_color f.φR_color]
	# true optimal model
	jj = 4 # 2
	selected_states_tru_ms = all_true_states[jj]
	ϕ_Ms_tru_ms = selected_states_tru_ms[:,7]
	ϕ_Rs_tru_ms = selected_states_tru_ms[:, 1]
	ϕ_ARs_tru_ms = selected_states_tru_ms[:, 6]
	all_ϕs_tru_ms = hcat(ϕ_ARs_tru_ms, ϕ_Ms_tru_ms, ϕ_Rs_tru_ms)
	labs2_ms = reshape([L"\phi_{AR}",L"\phi_M",L"\phi_R"], (1,3))
	areaplot(abx_rang2, (all_ϕs_tru_ms), legend=false, labels=labs2_ms, fillalpha = [0.7 0.7 0.7], seriescolor = prot_colors2, title="Optimal Proteome Allocation Cartoon", ylabel="",xlabel="Antibiotics [M]", yticks=false, size=(500,400), leftmargin=3mm)
end

# ╔═╡ cd4ca6c3-2b96-497f-a03f-8bbeac644d1e
begin
	# growth-regulated model
	jjj = 2
	selected_kcat_ms = kcats[jjj]
	selected_states_ms = allopt1states[jjj]
	ms_ϕ_Ms = selected_states_ms[:,7]*0.8
	ms_ϕ_Rs = selected_states_ms[:, 1]
	ms_ϕ_ARs = selected_states_ms[:, 6] * 20
	ms_ϕ_0s = pm_1.abx.φ_0 .* ones(size(selected_states_ms[:,1]))
	all_ϕs_ms = hcat(ms_ϕ_ARs, ms_ϕ_Ms, ms_ϕ_Rs)
	labs_ms = reshape([L"\phi_{AR}",L"\phi_M",L"\phi_R"], (1,3))
	p_colors = prot_colors2
	
	areaplot(abx_rang2, all_ϕs_ms, legend=false, labels=labs_ms, fillalpha = [0.7 0.7 0.7], seriescolor = p_colors, title="Optimal Ribosomal Allocation Cartoon", ylabel="",xlabel="Antibiotics [M]", yticks=false, ylims=[0, 1 - pm_1.abx.φ_0], size=(500,400), leftmargin=2mm)
end

# ╔═╡ 5ea4b05c-ed66-4082-a1aa-feb169baf2fb
md"""
# Fig S1
...
# ╔═╡ 3499e0de-2811-44de-9fb1-b81c762188e1
begin
	cpcs = range(0,30) .* params.abx.Km
	Rs = f.φ_R_heuristic.(cpcs, params.abx)
	heur_p = plot(cpcs, Rs, lw=3, label = "", xlabel = "Precursors [M]", ylabel=L"\phi_R", ylims=[0.0, 0.3], xlims=[0.0, 0.5])
end

# ╔═╡ ec7797e6-f894-40af-814e-34acf4f2b14f
begin
	pp3 = plot(xlabel="Growth Rate", ylabel=L"\phi_R")
	plot!(Ls2,Rs2, label="Empirical relation", lw = 3, fillalpha=0.3, color=1)#, ribbon=[0.02])
	plot!(analytic_opt_Ls, analytic_opt_Rs, label="Optimal steady-state solution", ls=:dashdot, lw = 3, fillalpha=0.3, color=2)#, ribbon=[0.02])
	scatter!(gr_df, phiR_df, label="Data collected in Chure & Cremer 2023", ms=3, ma = 0.7, markerstrokewidth=0., yerr = 0.02 * ones(length(phiR_df)), msc = 3, color=3)
	# change markerstrokewidth to > 0 to see the error bars representative of a typical error for these kinds of measurements
	# > 0.025 is a typical, but good (low) growth rate error for our lab (based on my measurements in the past)
end
...
manifest_format = "2.0"
project_hash = "1e71442e79b4066e6f912f4c2f8206b1b8293e0f"
