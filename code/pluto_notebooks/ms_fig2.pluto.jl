...
# ╔═╡ 1b3de3e6-6431-11f0-2a7d-57dbd670d383
begin
...
# ╔═╡ 4c917f3f-9e51-4549-bc08-b98c5dd7d74e
begin
	using JSON
	json_dict = JSON.parsefile("../secrets/paths_to_repos.json")
end

# ╔═╡ da09ae2e-18fc-474c-beb7-9a7d7b17d1a3
...
# ╔═╡ cc798499-01fd-4139-a34b-d9325cb54603
md"""
...
# ╔═╡ 031bc7b8-4c34-4518-ab25-0469ef0740d2
begin
...
# ╔═╡ 20cafa7c-423e-491d-aaa5-50d853199c87
md"""
# Figures 2 and S4
"""

# ╔═╡ 6a2a7c7d-ebb5-438e-a122-5b255af14458
md"""
# Fig 2
"""

# ╔═╡ 91075de5-eefc-4587-aa3f-64a219298ace
md"""
## 2A
"""

# ╔═╡ 034184bc-3c02-49cc-a81e-ab5efa629a86
md"""
## Susceptible Strategy 
"""

# ╔═╡ 3d5bfa4f-364c-4251-8a88-038be37b7c01
begin
	ϕ_R0 = 0.1
	ϕ_AR_0 = 0.0
	α_AR_0 = 0.0
	...
	T_phase = 20
	Tc_phase = T_phase/2
	tspan_phase = (-Tc_phase, 20)

	precis = 1e-7
	pm = f.default_PcABXwAnt_params()
	pm_noABX = f.mutate(pm, C_max = 0.0)
	pm_phase = f.mutate(pm, C_max = f.heaviside(f.Cm_ug_per_mL_to_M(5), 0), αmax = 0.00, αmin = 0.00, νmax=5.0, ke= 126000)
	prob_phase = ODEProblem(f.growth_dynamics, u0, tspan_phase, pm_phase)
	sol_phase = solve(prob_phase, Rodas4(), abstol = precis, reltol = precis)
		
	DE_plot_phase = f.plot_DE_soln(sol_phase, pm_phase)
	pq_phase1 = plot(DE_plot_phase...; layout = grid(1,4), size=(4*500, 500))
	for i in 1:length(pq_phase1)
		vline!(pq_phase1[i], [0], color=:black, ls=:dashdot, label="")
	end

	out = f.dSS_solver(pm_noABX)
	hline!(pq_phase1[1], [out[1]], color=:gray, ls=:dash, label="")
	hline!(pq_phase1[2], [out[2]], color=:black, ls=:dashdot, label="")
	hline!(pq_phase1[3], [out[3]/0.03], color=:black, ls=:dashdot, label="")
	hline!(pq_phase1[1], [out[7]], color=:gray, ls=:dashdot, label="")
	plot(pq_phase1)
end

# ╔═╡ 21ed2c80-98da-4df0-9a58-ab8278d0cad9
begin
	p3 = deepcopy(plot(pq_phase1[1]))
	plot!(minorgrid=true, gridalpha=0.2, minorgridalpha=0.1, tickfontsize=10, labelfontsize=12, legendfontsize=9,title="", xlims=(-3,20), xlabel="Time Since Antibiotic Exposure (hrs)", dpi=500, size=(500,400))
end

# ╔═╡ d22094fb-3ca9-42cc-b4ba-adcd981b6e6c
begin
	p2 = deepcopy(plot(pq_phase1[2]))
	plot!(minorgrid=true, gridalpha=0.2, minorgridalpha=0.1, tickfontsize=10, labelfontsize=12, legendfontsize=9, title="", xlims=(-3,20), xlabel="Time Since Antibiotic Exposure (hrs)", dpi=500, size=(500,400))
end

# ╔═╡ 251ad0c4-22be-49be-abce-b14f2a98470b
md"""
## 2B
"""

# ╔═╡ c0ac6fe8-0260-44b2-93d4-c43b57a01cec
begin
	abx_conc = 15 # ug/mL Cm
	pm_ABX = f.mutate(pm, C_max = f.Cm_ug_per_mL_to_M(abx_conc))
	# optimal ϕ_AR for given [ABX] (and other params)
	αmax_opt = f.ABX_optimizer_allOpt(pm_ABX)[end]
	pm_sweep = f.mutate(pm, αmax = αmax_opt, αmin = 0.0, C_max = f.heaviside(f.Cm_ug_per_mL_to_M(abx_conc), Tc_phase))
	ant_range = range(0, αmax_opt, length=4)

	sols = []
	grs = []
	ts = []
	for a in ant_range
		pm_sweep1 = f.mutate(pm_sweep, αmin = a)
		prob_sweep = ODEProblem(f.growth_dynamics, u0, tspan_phase, pm_sweep1)
		sol_sweep = solve(prob_sweep, Rodas4(), abstol = precis, reltol = precis)
		push!(sols, sol_sweep)
		push!(ts, sol_sweep.t)
		push!(grs, f.growth_rate(sol_sweep, pm_sweep))
	end
end

# ╔═╡ da667837-c25c-45fc-8852-86b1aaabc77c
begin
	# figure showing interpolation of anticipation from 0 to max (for 5 solns)
	p_gr_sweep = plot(ylabel="Rate (1/hr)", dpi=500, legend=:bottomright, ylims=(0,1.4), size=(500,400), xlims=(-3,7), leftmargin=0mm, xlabel="Time Since Antibiotic Exposure (hrs)", bottommargin=0mm)
	hline!([out[2]], color=:black, ls=:dashdot, label="")
	vline!([0], color=:black, ls=:dashdot, label="")
	end_col = colorant"orchid1"#"#D7B600"
	cs = f.gen_color_range(4, f.growth_color, end_col)
	for (i, a) in enumerate(ant_range)
		lab = ""
		if a == 0.0
			lab="Responsive"
		elseif a == αmax_opt
			...
			lab = "Anticipation = $(round(a, sigdigits=2))"
		end
		plot!(ts[i] .- Tc_phase, grs[i], label=lab, color=cs[i], lw=2, minorgrid=true, gridalpha=0.2, minorgridalpha=0.1, tickfontsize=10, labelfontsize=12, legendfontsize=9)
	end
	p_gr_sweep
end

# ╔═╡ 3f238513-3803-4969-8b44-044f21358546
begin
	# figure showing the α_AR command values across a shift paralleling graph above
	p_As_sweep = plot(ylabel="Antibiotic Resistance Mass Fraction", dpi=500, legend=:bottomright, ylims=(0,0.025), size=(500,400), xlims=(-3,7), xlabel="Time Since Antibiotic Exposure (hrs)")
	#hline!([out[2]], color=:black, ls=:dashdot, label="")
	vline!([0], color=:black, ls=:dashdot, label="")
	# ColorScheme(get(ColorSchemes.darkrainbow, 10 .^ range(log10(0.6), log10(0.3), length=5)))
	for (i, a) in enumerate(ant_range)
		lab = ""
		if a == 0.0
			lab="Responsive"
		...
		plot!(ts[i] .- 10, sols[i][6,:], label=lab, color=cs[i], lw=3, minorgrid=true, gridalpha=0.2, minorgridalpha=0.1, tickfontsize=10, labelfontsize=12, legendfontsize=9)
	end
	p_As_sweep
end

# ╔═╡ 5f6638b7-0a57-4d2b-9e91-bb6ec9c34af7
md"""
# Fig S4
"""

# ╔═╡ 5aabcdf4-e2a5-4346-8bb0-2eff2b2f6b30
md"""
## Constant Strategy
"""

# ╔═╡ fd408541-d078-4e38-be1f-eb7afc459749
begin
	const_AR = αmax_opt
	pm_const = f.mutate(pm, αmax = const_AR, αmin = const_AR)
	pm_noABX_const = f.mutate(pm_noABX, αmax = const_AR, αmin = const_AR)
	
	pm_const_heavi = f.mutate(pm_const, C_max = f.heaviside(f.Cm_ug_per_mL_to_M(abx_conc), Tc_phase))
	prob_const_heavi = ODEProblem(f.growth_dynamics, u0, tspan_phase, pm_const_heavi)
	sol_const_heavi = solve(prob_const_heavi, Rodas4(), abstol = precis, reltol = precis)
	DE_plot_const_heavi = f.plot_DE_soln(sol_const_heavi, pm_const_heavi)
	pq_const_heavi = plot(DE_plot_const_heavi...; layout = grid(2,2), size=(900, 700), legend=:topright)
	
	for i in 1:length(pq_const_heavi)
		vline!(pq_const_heavi[i], [Tc_phase], color=:black, ls=:dashdot, label="")
		plot!(legend=:outerright)
	end
	
	# validate ss soln
	out_ch = f.dSS_solver(pm_noABX)
	hline!(pq_const_heavi[1], [out_ch[1]], color=:black, ls=:dashdot, label="")
	hline!(pq_const_heavi[2], [out_ch[2]], color=:black, ls=:dashdot, label="")
	hline!(pq_const_heavi[3], [out_ch[3]/0.03], color=:black, ls=:dashdot, label="")
	hline!(pq_const_heavi[1], [out_ch[7]], color=:gray, ls=:dashdot, label="")
	plot(pq_const_heavi)
end

# ╔═╡ a8a1b57b-36e0-4ceb-a568-018268966c40
md"""
## Responsive Strategy 
"""

# ╔═╡ 6113ef2c-2f35-40fd-af34-f8fd005fc383
begin
	pm_resp = f.mutate(pm, αmax = αmax_opt, αmin=0)
	pm_resp_heavi = f.mutate(pm_resp, C_max = f.heaviside(f.Cm_ug_per_mL_to_M(abx_conc), Tc_phase))
	prob_resp_heavi = ODEProblem(f.growth_dynamics, u0, tspan_phase, pm_resp_heavi)
	sol_resp_heavi = solve(prob_resp_heavi, Rodas4(), abstol = precis, reltol = precis)
	DE_plot_resp_heavi = f.plot_DE_soln(sol_resp_heavi, pm_resp_heavi)
	pq_resp_heavi = plot(DE_plot_resp_heavi...; layout = grid(2,2), size=(900, 700), legend=:topright)
	for i in 1:length(pq_resp_heavi)
		vline!(pq_resp_heavi[i], [Tc_phase], color=:black, ls=:dashdot, label="")
		#plot!(legend=:outerright)
	end
	#
	# validate ss soln
	out_rh = f.dSS_solver(pm_noABX)
	hline!(pq_resp_heavi[1], [out[1]], color=:black, ls=:dashdot, label="")
	hline!(pq_resp_heavi[2], [out[2]], color=:black, ls=:dashdot, label="")
	hline!(pq_resp_heavi[3], [out[3]/0.03], color=:black, ls=:dashdot, label="")
	hline!(pq_resp_heavi[1], [out[7]], color=:gray, ls=:dashdot, label="")
	#
	#
	plot(pq_resp_heavi)
end

# ╔═╡ e48b02ae-53e7-4c43-a6d7-98d9a9a1c32a
md"""
## Anticipatory Strategy 
"""

# ╔═╡ 7f8f6800-6898-4787-84a5-f0561cf2f1d6
begin
	amin = 0.005
	pm_ant = f.mutate(pm, αmax = αmax_opt, αmin = amin)
	pm_ant_heavi = f.mutate(pm_ant, C_max = f.heaviside(f.Cm_ug_per_mL_to_M(abx_conc), Tc_phase))
	prob_ant_heavi = ODEProblem(f.growth_dynamics, u0, tspan_phase, pm_ant_heavi)
	sol_ant_heavi = solve(prob_ant_heavi, Rodas4(), abstol = precis, reltol = precis)
	DE_plot_ant_heavi = f.plot_DE_soln(sol_ant_heavi, pm_ant_heavi)
	pq_ant_heavi = plot(DE_plot_ant_heavi...; layout = grid(2,2), size=(900, 700), legend=:topright)
	for i in 1:length(pq_ant_heavi)
		vline!(pq_ant_heavi[i], [Tc_phase], color=:black, ls=:dashdot, label="")
		plot!(legend=:outerright)
	end
	#
	# validate ss soln
	out_ah = f.dSS_solver(pm_noABX)
	hline!(pq_ant_heavi[1], [out_ah[1]], color=:black, ls=:dashdot, label="")
	hline!(pq_ant_heavi[2], [out_ah[2]], color=:black, ls=:dashdot, label="")
	hline!(pq_ant_heavi[3], [out_ah[3]/0.03], color=:black, ls=:dashdot, label="")
	hline!(pq_ant_heavi[1], [out_ah[7]], color=:gray, ls=:dashdot, label="")
	#
	#
	plot(pq_ant_heavi)
end

# ╔═╡ bb02145a-7d07-431c-ab54-7b4b6f444bc1
begin
	#
	label = ["massFrac", "growth", "pc", "ABX"]
	i = 2
	#            1          , 2       , 3         , 4
	strats = ["Susceptible", "Constant", "Responsive", "Anticip"]
	j = 1
	#
	pplots = [pq_phase1, pq_const_heavi, pq_resp_heavi, pq_ant_heavi]
	pp = plot(pplots[j][i],title="",size=(500,400), legend=:topright,xlabel="")
	path = "/Users/lpz/Documents/R&D/PhD/anticipation/glyphs/"
	file_type = ".svg"
	#savefig(path * strats[j] * "_ABXstep_" * label[i] * file_type)
	print(strats[j] * "_ABXstep_" * label[i] * file_type)
end

# ╔═╡ eace044c-e369-45fa-920d-51294ac92e08
pplots
...
manifest_format = "2.0"
project_hash = "dc202001fd82c02b2d5ceb329063e33dc42333fa"
